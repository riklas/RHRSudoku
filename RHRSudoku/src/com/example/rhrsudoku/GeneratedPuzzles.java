...
public class GeneratedPuzzles implements SudokuGenerator {
	...
	int ceilingCount;
	int floorCount;
	ArrayList<int[]> removedValues = new ArrayList<int[]>();	//ArrayList used to store the removed values
	...
	public SudokuPuzzle removeValue(SudokuPuzzle targetPuzzle) {
		...
		int randx;
		int randy;
		int backtrackCount = 0;		//this sets a count for the number of tries before the generator backtracks on the stack (to prevent deadlock)
		...
		while (true) {
			...
			System.out.println("Removing from stack: " + removedValues.get(removedValues.size() - 1)[2]);
		...
		System.out.println("size: " + removedValues.size());
		int[] value;
		for (int i = 0; i < removedValues.size(); i++) {			
			 value = removedValues.get(i);
			 System.out.print("x:" + value[0] + ", y:" + value[1] + ", value:" + value[2]);
			 System.out.println();
		}
